...
function IndividualChallengeBoardItem(props){
    ...
    const [contents30List,setContents30List]=useState([]);
    ...
    async function getTasks(){
        ...
        const taskData=await axios.get(port.url + `/task/challenge/${challengeId}`);
        const filteredTaskData=taskData.data.filter((item)=>item.length!=0)
        console.log(taskData);
        console.log("filteredTaskData",filteredTaskData[0])
        setTasks(taskData);
        setContents30List(filteredTaskData[0]&&filteredTaskData[0].map((item)=>item.title))
        return taskData;
    }
